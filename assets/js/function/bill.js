		...
		$.ajax({
            ...
            success: function(data){
                ...
                for (var i = 0; i< data.length; i++) {
                   ...
                   tr += '<tr><td>' +data[i].id + 
                   '</td><td>' + data[i].totalPrice + 
                   '</td><td>'+ books + 
                   '</td><td>' + status +
                   '</td><td>' + data[i].createdDate + 
                   '</td><td>' + data[i].updatedDate + 
                   '</td><td><a href="#" class="btn btn-primary" id="id" onclick="editBill('+ data[i].id + ')">Edit</a> <a href="#" class="btn btn-danger" id="id" onclick="deleteBill('+ data[i].id + ')">Delete</a></td></tr>';
            }
                $('#tblbill').append(tr);
            },
            error: function(){
